  ...
  sessionsService: {
    ...
    getParticipants: async (sessionId) => {
      try {
        ...
        const sessionDetails = await module.exports.sessionsService.getSessionDetails(sessionId);
    ...
    broadcastMessage: async (sessionId, message) => {
      try {
        ...
        if (message.type === 'chat') {
          await module.exports.sessionsService.saveMessage(sessionId, message.content);
        }
        
        // If it's a system message, save it as a message
        if (message.type === 'system') {
          await module.exports.sessionsService.saveMessage(sessionId, {
            id: Date.now().toString(),
            sessionId,
            type: 'system',
